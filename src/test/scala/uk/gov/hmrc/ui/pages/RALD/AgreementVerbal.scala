...
object AgreementVerbal extends BasePage {
  ...
  def endDateInput(day: String, month: String, year: String): Unit =
    dateInput(day, month, year, "EndDate")

  private def dateInput(day: String, month: String, year: String, whichDate: String): Unit = {
    sendKeys(By.id(s"agreement$whichDate.day"), day)
    sendKeys(By.id(s"agreement$whichDate.month"), month)
    sendKeys(By.id(s"agreement$whichDate.year"), year)
  }
}
