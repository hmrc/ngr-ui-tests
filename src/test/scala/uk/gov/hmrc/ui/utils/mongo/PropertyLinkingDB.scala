...
object PropertyLinkingDB {
  ...
  def cleanup(): Unit =
    if (env == "local") {
      collection("next-generation-rates", "propertyLinkingUserAnswers").deleteMany(expr("1 == 1")).results()
      collection("ngr-property-linking-frontend", "propertyLinking").deleteMany(expr("1 == 1")).results()
    }
}
